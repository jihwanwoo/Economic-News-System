...
class ComprehensiveEconomicMonitor:
    ...
    def init_data_collectors(self):
        ...
        try:
            ...
            if alpha_api_key:
                self.alpha_collector = AlphaVantageIntelligenceComplete(alpha_api_key)
    ...
    async def collect_market_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.alpha_collector:
                try:
                    alpha_data = await self.alpha_collector.collect_intelligence_data()
    ...
    def calculate_sentiment_score(self, posts: List[Dict]) -> float:
        ...
        # 간단한 감정 분석 (키워드 기반)
        positive_keywords = ['bull', 'buy', 'up', 'gain', 'profit', 'good', 'great', 'excellent']
        negative_keywords = ['bear', 'sell', 'down', 'loss', 'crash', 'bad', 'terrible', 'awful']
    ...
    async def detect_events(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            new_events = []
            for event in events:
                if event not in self.events_history:
...
**시간**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    async def run_comprehensive_monitoring(self) -> Dict[str, Any]:
        ...
        try:
            ...
            market_data, economic_data, social_data, events = await asyncio.gather(
